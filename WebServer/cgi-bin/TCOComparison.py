...
def TCO_comparison(cores,storageTB,annualDBsupport,utilization, Exa_Core_Ratio=1):
    ...
    ExaCCResult=sizer.sizing(no_cores=cores,storage_needed=storageTB,use_DBServerExpansions=False,Exa_Core_Ratio=Exa_Core_Ratio)
    ExaCCInfraCost=sum(config["Cost"] for config in ExaCCResult)*12
    OnPremInfraCost=(cores*benchmarks["Cost of 1 core Intel Server Acquisition (USD)"]/benchmarks["Amortization Period in Years"]+
        storageTB*benchmarks["Cost of TB Storage CAPEX (USD)"]/benchmarks["Amortization Period in Years"]+
        cores*benchmarks["Support of 1 TB of Storage Annual OPEX (USD)"]+
        storageTB*benchmarks["OS and Server Annual Support for 1 Core (USD)"])
    UniversalCreditLI=0
